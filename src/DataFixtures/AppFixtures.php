...
class AppFixtures extends Fixture
{
    ...
    {
        $this->loadCatergories($manager);
        $this->loadBooks($manager);
        $this->loadPromotionCode($manager);
    }

    /**
     * @param ObjectManager $manager
     */
    private function loadCatergories(ObjectManager $manager){
        foreach ($this->getCatergoriesData() as $value){
    ...
    private function loadBooks(ObjectManager $manager){
        foreach ($this->getBooksData() as $value){

            $temp = $this->categoryRepository->getAllCategoryId();
            $category = $this->categoryRepository->find($temp[rand(0,count($temp)-1)]['id']);

            $book = new Book();
            $book->setTitle($value['title']);
            $book->setAuthor($value['authors'][0]);
            $book->setCategory($category);
            $book->setDescription($value['shortDescription']);
            $book->setImage($value['thumbnailUrl']);
            $book->setPrice(rand(100,5000));
            $book->setActive(1);
    ...
    private function loadPromotionCode(ObjectManager $manager){
        for ($i=0;$i<100;$i++){
            $code = new PromotionCode();

            $code->setCode(substr(md5(uniqid()),0,10));
            $code->setDiscount(0.15);
            $code->setActive(1);
     ...
     * @return array
     */
    private function getCatergoriesData():array {
        return [
            'Children',
            'Fiction',
            'Drama',
